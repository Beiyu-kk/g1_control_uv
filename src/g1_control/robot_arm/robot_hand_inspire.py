...
import logging_mp  # 轻量级python日志工具
...
class Inspire_Controller:
    ...
    def get_right_hand_state(self):
        ...
        if hand_msg is not None:
            ...
            for idx, id in enumerate(Inspire_Right_Hand_JointIndex):
                self.right_hand_state_array[idx] = hand_msg.states[id].q
        return self.right_hand_state_array
    
    def get_dual_hand_state(self):
        hand_msg  = self.HandState_subscriber.Read()
        if hand_msg is not None:
            for idx, id in enumerate(Inspire_Left_Hand_JointIndex):
                self.left_hand_state_array[idx] = hand_msg.states[id].q
            for idx, id in enumerate(Inspire_Right_Hand_JointIndex):
                self.right_hand_state_array[idx] = hand_msg.states[id].q
        return [self.right_hand_state_array , self.left_hand_state_array[idx]]
        
    
    def ctrl_right_hand(self, right_q_target):
        """
...
if __name__ == "main":
    ...
    hand_ctrl = Inspire_Controller(left_hand_pos_array, right_hand_pos_array, 
                                   dual_hand_data_lock, dual_hand_state_array, dual_hand_action_array, simulation_mode=False)
    right_q_target = [0.0]*12
    hand_ctrl.ctrl_right_hand(right_q_target)
    print( hand_ctrl.get_right_hand_state() )